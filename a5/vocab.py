...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        words = self.words2charindices(sents)
        padded_words =  pad_sents_char(sents=words,
                                     char_pad_token=self.char2id['<pad>'])
        # added dtype=torch.long. else wont train.
        output = torch.tensor(padded_words, dtype=torch.long, device=device).permute(1, 0, 2)
        return output
        ### END YOUR CODE
