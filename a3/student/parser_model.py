...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.zeros((self.embed_size * self.n_features, self.n_features)))
        self.embed_to_hidden_bias = nn.Parameter(torch.zeros((1, self.n_features)))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        nn.init.uniform_(self.embed_to_hidden_bias)

        self.dropout = nn.Dropout(p = dropout_prob)

        self.hidden_to_logits_weight = nn.Parameter(torch.zeros((self.n_features, self.n_classes)))
    ...
    def forward(self, w):
        ...
        w = F.relu(torch.matmul(w,self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        w = self.dropout(w)
        logits = torch.matmul(w,self.hidden_to_logits_weight) + self.hidden_to_logits_bias
