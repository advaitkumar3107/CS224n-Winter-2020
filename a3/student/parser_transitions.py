...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses):
    	...
    	for (parsed, transition) in zip(minibatch, transitions):
    		parsed.parse_step(transition)

    		if not parsed.buffer and len(parsed.stack) == 1:
	    		unfinished_parses.remove(parsed)
